...
class ConsensusMotionDetection:
    ...
    def update_motion_status(self, frame, mask, fps):
        ...
        mask_contour = self.motion_detector_contour.update_motion_status(frame, mask, fps)
        mask_sparse = self.motion_detector_sparse.update_motion_status(frame, mask, fps)
...
def iterate_main(main_dir='captures/videos'):
    ...
    for videopath in video_files[-2:]:
        ...
        while cap.isOpened():
            ...
            frame_count += 1
            if frame_count % 3 == 0:  # Process every 10th frame
